...
class ToolHost:
    ...
    def specs(self) -> list[dict[str, Any]]:
        """Tool specs filtered by the host's capabilities. Suitable for shipping
        to a provider as the agent's tool catalog."""
        return [
            {
                "name": t.name,
            ...
            for t in self._registry.available(capability=self._context.capabilities)
        ]
