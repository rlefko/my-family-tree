...
- **Tooling**: `uv`, `yarn`, `ruff` (lint+format), `ty` (typecheck), `pytest` + `testcontainers`, `vitest`, ESLint flat config + Prettier
...
- **TypeScript**: ESLint flat config + Prettier. `@/` alias for `src/`. TanStack Router file-based routes. TanStack Query for data fetching. `react-hook-form` + `zod` for forms.
...
- Don't use LiteLLM, axios, React Router (use TanStack Router), Biome (use ESLint), Black (use ruff), mypy (use ty), poetry / pip-tools (use uv).
